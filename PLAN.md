...
**Depth round (DP in BUGS.md).** The public demo became a realistically complex mid-size
decisioning workspace: 10 flows (23 published versions; every graph 8–14 nodes across 2–4 swimlanes;
all 14 node types with real configs), 420 decisions over ~32 days whose traces are generated by
WALKING the real graphs (a pure walk.ts core), 30 cases across every status/SLA state, a populated
agent/context/model/monitor/governance fleet (a genuinely-computed drift alert, 5 firing monitors,
pending four-eyes requests, schedules, grants, challenger arms, shadows) — 1.01MB persisted, six new
internal-consistency tests. Every page gained step-by-step in-page guidance (59 journeys across all
20 routes, numbered steps naming real controls, a filesystem-walk coverage test so a new page can't
ship guideless), and docs/JOURNEYS.md grew to 15 journeys. Boyscout: the long-standing dark-contrast
