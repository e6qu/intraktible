...
### AI / ML governance  (status: governed specialist-agent lifecycle + durable human/AI learning)
- **P1 — Governed agent assets, evaluation, deployment, and case assistance — ✅
  done.** The low-level immutable agent registry and direct/streaming run APIs remain
  available for development and flow AI-node dependencies. The production lifecycle
  adds reusable specialist templates; immutable releases with typed contracts,
  provider/model/tool/data/evidence pins, budgets, trust and human-gate policy;
  representative/adversarial versioned suites; repeated deterministic and governed
  semantic grading; human adjudication; exact-suite comparison and reproducible
  export; assigned independent release review; environment deployment, scheduling,
  pause, rollback, expiry, retirement, and explicit guarded resume.

  Durable case assists pin exact release and evidence snapshots, require visible
  citations, expose staleness and unsupported claims, resume human-before-call tools
  through replica-safe workers, and keep provider failure independent of queue/SLA/
  resolution. Reviewer accept/edit/reject evidence, value-free diffs, QA, validated
  outcomes, latency, tokens, and cost join into release/segment analytics and MRM.
  Event-derived circuits block admission and open safety incidents without provider
  fallback. Assist content and reviewer edits are subject-key sealed and
  crypto-shreddable while accountability proofs remain. A versioned remote-agent
  protocol preserves scoped identity, idempotency, cancellation, typed evidence, and
  platform-owned tool effects.
