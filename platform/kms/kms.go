...
// projects/p/locations/l/keyRings/r/cryptoKeys/k). Returns (nil, nil) when no
// provider is configured, so callers fall back to local keys.
func FromEnv(ctx context.Context) (KMS, error) {
	...
	if key == "" {
		return nil, fmt.Errorf("kms: INTRAKTIBLE_KMS_KEY is required for provider %q", provider)
	}
	switch provider {
	case "aws":
		return openAWS(ctx, key)
	case "gcp":
		return openGCP(ctx, key)
	default:
		return nil, fmt.Errorf("kms: unknown provider %q (aws|gcp)", provider)
	}
}
