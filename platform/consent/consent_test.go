...
func TestGrantAndWithdraw(t *testing.T) {
	h, log := handler(t0)
	if _, err := h.Grant(ctx, id, "cust-1", "credit_underwriting", consent.BasisConsent, nil); err != nil {
		t.Fatal(err)
	}
	st := build(t, log)
...
func TestConsentExpiry(t *testing.T) {
	...
	if _, err := h.Grant(ctx, id, "cust-1", "marketing", consent.BasisConsent, &exp); err != nil {
		t.Fatal(err)
	}
	st := build(t, log)
...
func TestGrantValidation(t *testing.T) {
	h, _ := handler(t0)
	if _, err := h.Grant(ctx, id, "", "p", consent.BasisConsent, nil); err == nil {
		t.Error("empty subject should fail")
	}
	if _, err := h.Grant(ctx, id, "s", "  ", consent.BasisConsent, nil); err == nil {
		t.Error("empty purpose should fail")
	}
	if _, err := h.Grant(ctx, id, "s", "p", consent.LawfulBasis("guess"), nil); err == nil {
		t.Error("unknown basis should fail")
	}
	past := t0.Add(-time.Hour)
	if _, err := h.Grant(ctx, id, "s", "p", consent.BasisConsent, &past); err == nil {
		t.Error("past expiry should fail")
	}
	// A blank basis defaults to explicit consent.
	if _, err := h.Grant(ctx, id, "s", "p", "", nil); err != nil {
...
func TestListSubjectPurposes(t *testing.T) {
	h, log := handler(t0)
	h.Grant(ctx, id, "cust-1", "credit_underwriting", consent.BasisContract, nil)
	h.Grant(ctx, id, "cust-1", "marketing", consent.BasisConsent, nil)
	h.Grant(ctx, id, "cust-2", "credit_underwriting", consent.BasisConsent, nil) // different subject
	st := build(t, log)
