...
func TestInboxFromMentions(t *testing.T) {
	...
	s := project()
	mine, err := notifications.List(ctx, s, reviewer, notifications.Access{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if authorInbox, _ := notifications.List(ctx, s, author, notifications.Access{}); len(authorInbox) != 0 {
	...
	after, _ := notifications.List(ctx, project(), reviewer, notifications.Access{})
