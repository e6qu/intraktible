...
func (s *Scheduler) Tick(ctx context.Context) (TickSummary, error) {
	...
	for _, k := range order {
		...
		if len(fired) > 0 && s.notifier != nil {
			...
			summary, err := s.notifier.Deliver(ctx, id, "monitor scheduler", payload)
			if err != nil || summary.RetryWorthy() {
				// A real error or a retryable total failure: leave the firing-edge alert
				// unrecorded so the next tick re-delivers, and don't starve the sweep.
				slog.Warn("monitor scheduler: delivery not completed, will retry next tick", "flow", k.flow, "err", err)
