...
func TestSummarize(t *testing.T) {
	...
	if s.ByStatus[domain.StatusNeedsReview] != 2 || s.ByStatus[domain.StatusInProgress] != 1 || s.ByStatus[domain.StatusCompleted] != 1 {
		t.Errorf("ByStatus = %v", s.ByStatus)
	}
	if s.Unassigned != 2 {
		t.Errorf("Unassigned = %d, want 2", s.Unassigned)
	}
	if s.DueSoon != 1 {
